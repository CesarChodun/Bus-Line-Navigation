...
//Komentarz wyjatkowo w jezyku polskim aby zachowac jednorodnosc w pliku(map.h).
/** @brief Tworzy droge krajowa przechodzaca przez podane miasta.
 ...
 * ale ma wcześniejszy rok budowy lub ostatniego remontu,
 * to modyfikuje ten atrybut odcinka drogi. 
 * @param[in, out] map  - wskaznik na mape drog;
 * @param[in] num       - numer drogi krajowej;
 * @param[in] cityNames - wskaznik na tablice nazw miast(C type string);
 * @param[in] roadLengths - wskaznik na tablice dlugosci kolejnych drog;
 * @param[in] roadBuiltYears - wskaznik na tablice dat remontu/budowy kolejnych drog.
 * @return Wartosc @p true, jesli droga krajowa zostala utworzona.
 * Wartosc @p false, jesli wystapil blad: ktorys z parametrow ma nie poprawna wartosc,
 * ktoras z drog istnieje i ma inna dlugosc lub pozniejszy rok budowy/remontu,
 * albo nie udalo sie zaalokowac pamieci.
 */
 ...
 * A w przeciwnym przypadku niczego nie zmienia w mapie dróg.
 * Nie usuwa odcinków dróg ani miast.
 * @param[in, out] map  - wskaźnik na strukturę przechowującą mapę dróg;
 * @param[in] routeId   - numer drogi krajowej.
 * @return Wartość @p true, gdy droga krajowa istnieje i została usunięta.
 * @p false, gdy podana droga krajowa nie istnieje lub podany numer jest niepoprawny.
 */
